package 
{
	...
	public class Main extends Sprite 
	{
		...
		private var _black:DraggableSprite;
		//保持被拖动对象原始坐标
		...
		public function place(event:MouseEvent):void {
			...
			//Get a list of objects inside this container that are underneath mouse
			var dropTargets:Array = getObjectsUnderPoint(new Point(mouseX,mouseY));
			//检测是否已经拖动到白色矩形上
			if (dropTargets[dropTargets.length-2] == _black) {
			    var color:uint;
				switch(event.target) {
